...
function drawRectsObservable() {
    const svg = document.getElementById("drawRects"), mousedrag = Observable.fromEvent(svg, 'mousedown'), svgBounds = svg.getBoundingClientRect();
    mousedrag.
        map(({ clientX, clientY }) => ({ initX: clientX - svgBounds.left, initY: clientY - svgBounds.top }))
        .map(({ initX, initY }) => ({ rect: new Elem(svg, "rect")
            .attr('x', initX)
            .attr('y', initY)
            .attr('width', '5')
            .attr('height', '5')
...
function drawAndDragRectsObservable() {
    const svg = document.getElementById("drawAndDragRects"), mousedrag = Observable.fromEvent(svg, 'mousedown'), svgBounds = svg.getBoundingClientRect();
    mousedrag.
        map(({ clientX, clientY }) => ({ initX: clientX - svgBounds.left, initY: clientY - svgBounds.top }))
        .map(({ initX, initY }) => ({ rect: new Elem(svg, "rect")
            .attr('x', initX)
